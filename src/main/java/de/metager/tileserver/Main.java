...
public class Main {
	...
	public static void main(String[] args) {
		...
		if (!tileCachePath.canWrite() || !tileCachePath.isDirectory()) {
			System.err.println("Cannot write to the supplied path or it's not a directory. Sorry!");
			System.exit(-1);
		} else {
			File tileCache = new File(tileCachePath, "tiles");
			if (!tileCache.isDirectory() || !tileCache.exists()) {
				tileCache.mkdirs();
				tileCache = null;
			}
		}
		...
		// either
		File mapFilePath = new File(args[1]);
		...
		File renderThemeFile = new File(args[2]);
		...
		CacheManager cacheManager = new CacheManager(tileCachePath, mapFilePath, renderThemeFile);
		Thread cacheManagerThread = new Thread(cacheManager);
		cacheManagerThread.start();

		int threadcount = 1;
		ArrayList<Thread> threads = new ArrayList<>();
		ArrayBlockingQueue<File> bq = new ArrayBlockingQueue<>(1024);
		for (int i = 0; i < threadcount; i++) {
			threads.add(new Thread(new TileRenderer(tileCachePath, mapFilePath, renderThemeFile, bq)));
			threads.get(i).start();
		}

		// Get Temp Directory for communication
		File tmpDir = new File(System.getProperty("java.io.tmpdir"));
		if (!tmpDir.canWrite()) {
			System.err.println("Cannot write to Tmp Directory: " + tmpDir.getAbsolutePath());
			return;
		}
		tmpDir = new File(tmpDir, "tile_requests");
		if (!tmpDir.exists()) {
			tmpDir.mkdirs();
		}
		
		Path path = Paths.get(tmpDir.getAbsolutePath());
		try {
			...
			while (true) {
				try {
					WatchKey key = watchService.poll(10L, TimeUnit.SECONDS);
					...
					for (WatchEvent<?> event : events) {
						...
						if(kind == StandardWatchEventKinds.OVERFLOW) {
							// We must have missed some events
							// Stop Listening to events
							watchKey.cancel();
							watchService.close();
							// Fetch a List of all Files and manually put them to the queue
